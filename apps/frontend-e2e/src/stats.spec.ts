...
test.describe('Stats and Calendar Flow', () => {
  ...
  test('should navigate to stats page', async ({ page }) => {
    ...
    const statsLink = page.locator('a:has-text("Stats"), a:has-text("Statistiques"), [href="/stats"]');
  ...
  test('should display stats summary for new user', async ({ page }) => {
    ...
    const statsSection = page.locator('.stats, .stat-card, [class*="stat"]').first();
  ...
  test('should display calendar', async ({ page }) => {
    ...
    const monthDisplay = page.locator('text=/janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre/i');
    await expect(monthDisplay).toBeVisible();
  });

  test('should navigate calendar months', async ({ page }) => {
    await page.goto('/stats');
    await page.waitForSelector('.calendar, [class*="calendar"]', { timeout: 5000 });

    // Get initial month
    const monthDisplay = page.locator('text=/janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre/i').first();
    const initialMonth = await monthDisplay.textContent();

    // Click next month button
    const nextButton = page.locator('button:has-text(">"), button[aria-label*="next"], button[aria-label*="suivant"]').first();
  ...
  test('should display stats after completing a routine', async ({ page }) => {
    ...
    let attempts = 0;
    while (attempts < 10) {
      const finishButton = page.locator('button:has-text("Terminer"), button:has-text("Compléter")');
  ...
  test('should show calendar days with activity', async ({ page }) => {
    ...
    const activityDay = page.locator('.has-activity, .completed, [class*="active"]').first();
